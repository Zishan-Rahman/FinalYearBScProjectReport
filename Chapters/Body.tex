...
Initially having done the noise integration with a sprite node and noise texture allowed me to experiment with some of the \verb|FastNoiseLite| class's properties. An instance of this class, by default, uses the ``Simplex Smooth" noise algorithm, a version of the Simplex algorithm that produces higher quality noise images at the expense of slower speed. We can also use just ``Simplex" noise for higher speed, as well as the original ``Perlin" noise algorithm. 
